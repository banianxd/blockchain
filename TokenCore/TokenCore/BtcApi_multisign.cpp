...
struct TransactionInput
{
	...
	string script;
	uint32_t sequence;
	string str_redeem_script;
	vector<string> vec_sign;
};
...
void tx_decode(bool is_testnet, const string tx_str, TransactionBill& tb)
{
	...
	for (size_t i = 0; i < input_count; i++)
	{
		...
		string str_redeem_script_len = tx_str.substr(index, 2);
		uint8_t redeem_script_len = string_to_uint8_t(str_redeem_script_len);
		if (0xff != redeem_script_len) {
			index += 2;
			in.str_redeem_script = tx_str.substr(index, redeem_script_len * 2);
			index += redeem_script_len * 2;
		}
...
string tx_encode(const TransactionBill& tb)
{
	...
	for (size_t i = 0; i < tb.inputs.size(); i++)
	{
		...
		if ("" == tb.inputs[i].script)
			...
			//string str_sign_script_len = uint8_t_to_string(u8_sign_script_len);
		...
		/*if (tb.inputs[i].str_redeem_script != "") {
			uint8_t u8_redeem_script_len = (uint8_t)tb.inputs[i].str_redeem_script.size() / 2;
			str_encode += uint8_t_to_string(u8_redeem_script_len);
			str_encode += tb.inputs[i].str_redeem_script;
		}*/
...
string str_reverse(const std::string &str) {
		return reverse_big_little_endian(str);
	}
...
string get_redeem_script(const std::vector<std::string> &vec_pubkey,
	...
	m += 80;
	n += 80;
	std::string str_redeem_script = uint8_t_to_string(m);
...
string signature_secp256k1(const string& str_prikey, const string& str_hash)
{
	if (str_prikey.size() != 64 || str_hash.size() != 64)
		return std::string("");

	string str_sign;
	Binary sz_hash = Binary::decode(str_hash);
	Binary sz_prikey = Binary::decode(str_prikey);

	secp256k1_context* pContext = secp256k1_context_create(SECP256K1_CONTEXT_SIGN);
	secp256k1_ecdsa_signature secp256k1_sign;
	int nRet = secp256k1_ecdsa_sign(pContext, &secp256k1_sign, sz_hash.data(),
		sz_prikey.data(), NULL, NULL);
	if (1 == nRet)
	{
		unsigned char sz_sign[1024] = { 0 };
		size_t sign_len = 1024;
		nRet = secp256k1_ecdsa_signature_serialize_der(pContext, sz_sign, &sign_len, &secp256k1_sign);
		if (1 == nRet)
		{
			Binary sign_tmp(sz_sign, sign_len);
			str_sign = Binary::encode(sign_tmp);
		}
	}
...
string sign_input_redeem(bool is_testnet, const string& str_tx, 
	const int input_index, const string& input_script,
	const string& private_key, const std::string &str_redeem_script)
{
	TransactionBill tx;
	tx_decode(is_testnet, str_tx, tx);
	//tx.inputs[input_index].script = input_script;
	//tx.inputs[input_index].script = input_script + str_redeem_script;
	tx.inputs[input_index].script = str_redeem_script;

	uint32_t u32_sign_type = 1;
	string str_script_tx = tx_encode(tx) + uint32_t_to_little_endian(u32_sign_type);
	string str_hash = Binary::encode(bitcoin256(Binary::decode(str_script_tx)));
	string str_sign = signature_secp256k1(private_key, str_hash);
	// str_sign = sig_serialize(str_sign) + "01";
	str_sign += "01";
...
int make_unsign_tx_redeem(UserTransaction* ut, const std::string &str_redeem_script)
{
	...
	for (size_t i = 0; i < ut->utxo_list.size(); i++)
	{
		...
		if (ut->utxo_list[i].script.size() == 56)
			input.str_redeem_script = str_redeem_script;
...
void sign_tx(bool is_testnet, UserTransaction* ut, const string& private_key)
{
	...
	for (int i = 0; i < ut->input_count; i++)
	{
		...
		string sign_data = sign_input(is_testnet, ut->tx_str, i, 
			prev_utxo_script, private_key);

		//ut->vec_unlock_script[0]
		if (ut->input_sign_data.size() == i) {
			vector<Binary> vec_sign;
			vec_sign.push_back(Binary::decode(sign_data));
			ut->input_sign_data.push_back(vec_sign);
		}
		else {
			ut->input_sign_data[i].push_back(Binary::decode(sign_data));
		}
		//ut->input_sign_data[i].push_back(sign_data);

		/*if (tx.inputs[i].str_redeem_script != "")
			tx.inputs[i].vec_sign.push_back(sign_data);
		else
...
void sign_tx_redeem(bool is_testnet, UserTransaction* ut,
	const string &private_key, const std::string &str_redeem_script) {
	if (NULL == ut || "" == private_key || "" == str_redeem_script) {
		return;
	}
	...
	{
		string prev_utxo_script = encode_script_(ut->utxo_list[i].script);
		string sign_data = sign_input_redeem(is_testnet, ut->tx_str, i,
			prev_utxo_script, private_key, str_redeem_script);
		std::cout << "prikey:\n" << private_key << std::endl;
		std::cout << "redeem_script:\n" << str_redeem_script << std::endl;
		std::cout << "prev_utxo_script\n" << prev_utxo_script << std::endl;
		std::cout << "sign_data:\n" << sign_data << std::endl;
...
void make_sign_tx(bool is_testnet, UserTransaction* ut)
{
	...
	for (int i = 0; i < ut->input_count; i++)
	{
		...
		if (1 == ut->input_sign_data[i].size()) {
			string str_sign = Binary::encode(ut->input_sign_data[i][0]);
			str_script += uint8_t_to_string((uint8_t)str_sign.size() / 2);
			str_script += str_sign;
			str_script += uint8_t_to_string((uint8_t)ut->public_key.size() / 2);
			...
			str_script += str_signs;
			str_script += "4c";

			//string str_redeem_script = tx.inputs[i].str_redeem_script;
			//string str_redeem_script = "522102d7383b18a3d62fb0ef02e27770638ae05d29b87a6986f00ebb968fe1a04616fe21022592d7c6bb03eb371e8e667241a8a157e8390af9d61be9856a770f3b0b3c587721036966acf2e64741b87a8ef2e26636a5b63b180d25385ccb9620589cac63e4b28553ae";
			//str_script += get_tag_len((uint8_t)str_redeem_script.size() / 2);
			//str_script += str_redeem_script;
		}

		//uint64_t script_len = (uint64_t)str_script.size();
		//string str_tag_len = get_tag_len(script_len) / 2;
		//str_script = str_tag_len + str_script;
