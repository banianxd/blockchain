...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_btxon_tokencore_Common__1get_1sha(JNIEnv *env, jclass type, jstring _data) {
    const char *data_r = env->GetStringUTFChars(_data, 0);
    std::string pro_data_str = data_r;
    std::string result = get_sha(pro_data_str);
    jstring ra = env->NewStringUTF(result.c_str());
    env->ReleaseStringUTFChars(_data, data_r);
    return ra;
}
